document.addEventListener("DOMContentLoaded", () => {
  ...
  function isBadResolvedMatch(input, display) {
    const inputNorm = normalize(input);
    const displayNorm = normalize(display);

    const saddleBrookWanted = inputNorm.includes("saddle brook") || inputNorm.includes("07663");
    if (saddleBrookWanted && !displayNorm.includes("saddle brook")) return true;

    const fairviewWanted = inputNorm.includes("fairview") || inputNorm.includes("07022");
    if (fairviewWanted && !displayNorm.includes("fairview")) return true;

    const hobokenWanted = inputNorm.includes("hoboken") || inputNorm.includes("07030");
    if (hobokenWanted && !displayNorm.includes("hoboken")) return true;

    const newarkWanted = inputNorm.includes("newark") || inputNorm.includes("07105");
    if (newarkWanted && !displayNorm.includes("newark")) return true;

    const zip = extractZip(inputNorm);
    if (zip && !displayNorm.includes(zip)) return true;
  ...
  async function geocodeAddress(address) {
    const cleaned = cleanGroupLine(address);
    ...
    const scored = candidates
      .map((item) => {
        ...
        if (blob.includes("new jersey")) score += 150;
        if (String(addr.state || "").toLowerCase() === "new jersey") score += 200;

        if (zipHint && String(addr.postcode || "") === zipHint) score += 450;

        const chunks = inputNorm.split(" ").filter(Boolean);
        for (const c of chunks) {
          if (blob.includes(c)) score += 20;
        }

        if (inputNorm.includes("hoboken") && blob.includes("hoboken")) score += 300;
        if (inputNorm.includes("newark") && blob.includes("newark")) score += 300;
        if (inputNorm.includes("fairview") && blob.includes("fairview")) score += 300;
        if (inputNorm.includes("saddle brook") && blob.includes("saddle brook")) score += 350;

        if (blob.includes("monmouth") && !inputNorm.includes("monmouth")) score -= 250;
        if (blob.includes("manalapan") && !inputNorm.includes("manalapan")) score -= 250;
        if (blob.includes("middletown") && !inputNorm.includes("middletown")) score -= 250;
        if (blob.includes("staten island") && !inputNorm.includes("staten island")) score -= 250;
        if (blob.includes("new york") && !inputNorm.includes("new york")) score -= 250;
    ...
    if (isBadResolvedMatch(cleaned, best.display_name || "")) {
      throw new Error(`Address not found: ${cleaned}`);
    }
  ...
  function classifyVenue(tags = {}) {
    ...
    if (amenity === "restaurant" && (text.includes("bar") || text.includes("lounge") || text.includes("dance"))) {
  ...
  function renderGroupResults(mid, midpointInfo, places, groupGeos, mode, outlierNote = "") {
    ...
    els.results.innerHTML = `
      ...
      <div class="card">
        ...
        ${
          outlierNote
            ? `<div class="card warn-card"><p style="margin:0;"><b>Possible bad match:</b> ${escapeHtml(outlierNote)}</p></div>`
            : ""
        }
        <div class="result-links">
  ...
  function wireModeButtons() {
    document.querySelectorAll("[data-mode]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        try {
          if (els.results) {
            els.results.innerHTML = `<p>Loading ${escapeHtml(mode)} near ${escapeHtml(lastCenter.midpointInfo.town)}...</p>`;
          }
  ...
  async function meetInMiddle() {
    ...
    try {
      ...
      lastCenter = { kind: "middle", mid, midpointInfo, outlierNote: "" };
      lastMode = "nightlife";
        ...
        els.results.innerHTML = `
          <div class="card error-card">
            ...
            <p style="margin:10px 0 0;">
              Try one clean location per line like:<br>
  ...
  function loadNJVenues() {
    ...
    els.results.innerHTML = `
      ...
      <div class="card">
        <p style="margin:0;">
          <b>Tip:</b> Use clean entries like <b>Hoboken NJ 07030</b> or <b>137 Fleming Ave, Newark, NJ 07105</b>.
        </p>
      </div>
    `;
  }
