...
`;
  ...
  async function meetInMiddle() {
    ...
    try {
      ...
      els.results.innerHTML = `
        <div class="card error-card">
          <p style="margin:0;"><b>Load failed.</b> One of the two locations could not be matched cleanly. Try town + NJ + ZIP.</p>
          <p style="margin:8px 0 0;">${escapeHtml(err.message || "Meet in the middle failed")}</p>
        </div>
      `;
    }
  }
  ...
  async function groupCenter() {
    ...
    try {
      ...
      if (good.length < 2) {
        els.results.innerHTML = `
          <div class="card error-card">
            <p style="margin:0;"><b>Load failed.</b> I could not cleanly match enough locations.</p>
            ${
              bad.length
                ? `<p style="margin:8px 0 0;"><b>Problem lines:</b><br>${bad.map(escapeHtml).join("<br>")}</p>`
                : ""
            }
          </div>
        `;
      ...
      if (bad.length) {
        els.results.innerHTML += `
          <div class="card warning-card">
            <p style="margin:0;"><b>Skipped lines:</b><br>${bad.map(escapeHtml).join("<br>")}</p>
          </div>
        `;
      }
    } catch (err) {
